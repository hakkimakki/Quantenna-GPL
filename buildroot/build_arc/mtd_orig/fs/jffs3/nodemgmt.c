 ...
 *
 * JFFS2 Id: nodemgmt.c,v 1.115 2004/11/22 11:07:21 dwmw2  Exp
 * $Id: nodemgmt.c,v 3.13 2005/01/05 16:19:00 dedekind Exp $
 *
 */
 ...
 *
 *	If it returns zero, jffs3_reserve_space() also downs the per-filesystem
 *	allocation semaphore, to prevent more than one allocation from being
 *	active at any time. The semaphore is later released by jffs3_commit_allocation()
 *
 *	jffs3_reserve_space() may trigger garbage collection in order to make room
 ...
 restart:
	...
	if (!jeb) {
		...
		if (list_empty(&c->free_list)) {
			...
			if (!c->nr_erasing_blocks &&
				...
				ejeb = list_entry(c->erasable_list.next, struct jffs3_eraseblock, list);
				list_del(&ejeb->list);
			...
			jffs3_erase_pending_blocks(c, 1);
			spin_lock(&c->erase_completion_lock);
		...
		list_del(next);
		c->nextblock = jeb = list_entry(next, struct jffs3_eraseblock, list);
		c->nr_free_blocks--;
	...
	*len = jeb->free_size - nofree_size;
	...
	if (c->cleanmarker_size && jeb->used_size == c->cleanmarker_size &&
		...
		*/
		spin_unlock(&c->erase_completion_lock);
		jffs3_mark_node_obsolete(c, jeb->first_node);
		spin_lock(&c->erase_completion_lock);
	}
...
/**
 *	jffs3_add_physical_node_ref - add a physical node reference to the list
 *	@c: superblock info
...
void jffs3_mark_node_obsolete(struct jffs3_sb_info *c, struct jffs3_raw_node_ref *ref)
{
	struct jffs3_eraseblock *jeb;
	int blocknr;
	...
	if (jffs3_can_mark_obsolete(c) && !jffs3_is_readonly(c) &&
		...
		/* Hm. This may confuse static lock analysis. If any of the above
		   ...
		   jffs3_raw_node_ref without the erase_completion_lock. */
	...
	if (ref != jeb->first_node ) {
		struct jffs3_raw_node_ref *p = jeb->first_node;
